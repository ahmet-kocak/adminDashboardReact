...
const navigate=useNavigate()
  ...
  const login=JSON.parse(localStorage.getItem("jwtToken"))
    ...
    <>
      ...
      <MenuPopover
        ...
        <Box sx={{ my: 1.5, px: 2.5 }}>
          <Typography variant="subtitle2" noWrap>{login.firstName}</Typography>
          <Typography variant="body2" sx={{ color: 'text.secondary' }} noWrap>{login.email}</Typography></Box>
        ...
        <MenuItem onClick={()=>{navigate('/');localStorage.removeItem("jwtToken");localStorage.removeItem("cdtl");
           window.location.reload() }} sx={{ m: 1 }}>Logout</MenuItem>
      </MenuPopover>
    </>
  );
}
